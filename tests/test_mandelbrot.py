...
def test_linspace_small_matches_numpy():
    """Small linspace implementation should match numpy.linspace for a few cases."""
    a = linspace(-1.0, 1.0, 5)
    b = np.linspace(-1.0, 1.0, 5).tolist()
    assert a == b
...
def test_mandelbrot_grid_shape_and_values():
    ...
    # spot-check a few entries by recomputing the point function
